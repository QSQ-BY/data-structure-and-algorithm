...
他想知道如果他希望在每个小山丘的顶部放置一个守卫，他需要多少守卫。他有一张地图，这张地图是一个整数矩阵；矩阵有 N 行（1<N≤700）和 M 列（1<M≤700）。矩阵中的每个元素表示一个高度 H 
ij
​
 （0≤H 
ij
​
 ≤10,000）。请帮助他确定地图上有多少个山顶。
...
* 第 2 行到第 N+1 行：第 i+1 行描述矩阵的第 i 行，包含 M 个用空格分隔的整数：H 
ij
​
    ...
    for j in range(m):
        ...
        while(stack):
            ...
            for (dx,dy) in directions:
                ...
                if(g[x][y] >g[i][j]):
                    is_peak = False
                elif(g[x][y] == g[i][j] and vis[x][y] == False):
                    vis[x][y] = True
                    stack.append((x,y))
        if(is_peak == True):
            ans += 1
