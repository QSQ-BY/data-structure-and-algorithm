...
2 4
说明/提示
对于 100% 的数据，满足初始时，没有两个士兵同在一个坐标，1≤L≤5×10 
3
 ，0≤N≤5×10 
3
 ，且数据保证 N≤L。 """
...
def test01():
    ...
    for pos in positions:
        mid = length//2
        if(pos > mid):
            minimum_step = max(length - pos + 1,minimum_step)
        else:
            minimum_step = max(pos,minimum_step)
    ...
    if(n==1):
        pos = positions[0]
        maximum_step = max(pos , length - pos + 1)
        print(f"{minimum_step} {maximum_step}")
        sys.exit()
    positions.sort()
    for i in range(n):
        cur = positions[i]
